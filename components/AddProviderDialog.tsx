...
export function AddProviderDialog({
  ...
  const [providerType, setProviderType] = useState<"standard" | "cloudflare">(
    "standard"
  );
  ...
  const fetchModels = async () => {
    if (providerType === "cloudflare") {
      toast(
        "Cloudflare models must be entered manually (e.g., @cf/meta/llama-3.1-8b-instruct)"
      );
      return;
    }
  ...
  const handleCreate = async () => {
    ...
    if (providerType === "cloudflare") {
      ...
      if (!manualModels.trim()) {
        toast.error("Please enter at least one model (comma-separated)");
        return;
      }
    }
    ...
    try {
      ...
      if (providerType === "cloudflare") {
        providerData.workerUrl = workerUrl.trim();
        providerData.maxIndex = parseInt(maxIndex);
        providerData.currentIndex = 0;
      } else {
        providerData.baseUrl = apiUrl.trim();
      }
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Select
              value={providerType}
              onValueChange={(value: "standard" | "cloudflare") =>
          ...
          <div>
            ...
            <Input
              ...
              placeholder={
                providerType === "cloudflare"
                  ? "e.g., My Cloudflare AI"
                  : "e.g., OpenAI, Groq, Cerebras"
              }
            />
          </div>
          ...
          {providerType === "cloudflare" ? (
            <div>
              <Label htmlFor="manualModels">Models (comma-separated) *</Label>
              <Input
                id="manualModels"
                value={manualModels}
                onChange={(e) => setManualModels(e.target.value)}
                placeholder="@cf/meta/llama-3.1-8b-instruct, @cf/mistral/mistral-7b-instruct-v0.1"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Enter Cloudflare model names separated by commas
              </p>
            </div>
          ) : (
            <div>
                ...
                <div className="mt-3 border rounded-lg overflow-hidden">
                  <div className="bg-muted px-3 py-2 border-b">
                    <p className="text-sm font-semibold">
                      Found {fetchedModels.length} models
                    </p>
                  </div>
